...
ANY_COUNT=$(find "$TARGET_DIR" -type f \( -name "*.ts" -o -name "*.tsx" \) \
  ! -name "*.d.ts" \
  ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" ! -path "*/build/*" \
  -exec grep -ohE '\bany\b|as any' {} + 2>/dev/null | wc -l | tr -d ' ')

# Count TODO, FIXME, HACK, XXX comments
TODO_COUNT=$(find "$TARGET_DIR" -type f \
  ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" ! -path "*/build/*" \
  -exec grep -ohE '(TODO|FIXME|HACK|XXX)' {} + 2>/dev/null | wc -l | tr -d ' ')

# Count eslint-disable comments
ESLINT_DISABLE_COUNT=$(find "$TARGET_DIR" -type f \( -name "*.js" -o -name "*.ts" -o -name "*.jsx" -o -name "*.tsx" \) \
  ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" ! -path "*/build/*" \
  -exec grep -ohE 'eslint-disable' {} + 2>/dev/null | wc -l | tr -d ' ')

# Count @ts-ignore and @ts-nocheck
TS_IGNORE_COUNT=$(find "$TARGET_DIR" -type f \( -name "*.ts" -o -name "*.tsx" \) \
  ! -path "*/node_modules/*" ! -path "*/.git/*" ! -path "*/dist/*" ! -path "*/build/*" \
  -exec grep -ohE '@ts-(ignore|nocheck)' {} + 2>/dev/null | wc -l | tr -d ' ')
