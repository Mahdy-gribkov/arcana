...
function getCategoryFor(skillName: string): string | undefined {
...
async function skillDetailFlow(
  skillName: string,
  allSkills: SkillInfo[],
  providerName: string,
): Promise<"back" | "menu"> {
...
async function searchFlow(allSkills: SkillInfo[], providerName: string): Promise<void> {
  while (true) {
...
async function quickSetup(allSkills: SkillInfo[], providerName: string): Promise<void> {
  ...
  const notInstalled = validSuggestions.filter((s: string) => !isSkillInstalled(s));
  if (notInstalled.length === 0) {
    p.log.success("All recommended skills are already installed.");
    return;
  }

  const options = validSuggestions.map((name: string) => {
    const installed = isSkillInstalled(name);
    return {
      value: name,
  ...
  const toInstall = (selected as string[]).filter((s: string) => !isSkillInstalled(s));
...
async function manageInstalled(allSkills: SkillInfo[], providerName: string): Promise<void> {
  while (true) {
...
async function updateAll(providerName: string): Promise<void> {
  ...
  for (const u of updates) {
    try {
      ...
      if (err instanceof Error) p.log.error(`Failed to update ${u.name}: ${err.message}`);
    }
  }
...
function showCliReference(): void {
  const ref = [
